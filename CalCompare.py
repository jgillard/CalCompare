...
HoD = ['9am', '10am', '11am', '12pm', '1pm', '2pm',
...
def getParseCal(people):
    ...
    for i in rcal.walk('VEVENT'):
        ...
        if temp['DTSTART'].hour != 8:
            events.append(temp)
        try:
            # if an RRULE is present, produce COUNT-1 number of extra
            # individual events
            count = int(str(i['RRULE']['COUNT'])[1:-1])
            # day = i['RRULE']['BYDAY']        # DOUBLE-CHECK THE DAY IS RIGHT
            # FROM +WEEK
